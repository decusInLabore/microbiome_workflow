---
title: "Bulk RNA-Seq Correlation Module"
...
setGeneric(
    ...
    ## Create group list ##
    #group.list <- createGroupList(dfDesign)
    ## Reorder group List
    plotList <- list()
    chnkVec <- as.vector(NULL, mode = "character")
    if (is.null(dfDesign)){
    ...
    for (i in 1:nrow(df.gmt)) {
        ######################################################
        ## Start creating one plot per gmt row/gene set
        ## Set parameters for this plot
        ...
        for (k in 1:length(group.list)) {
            ...
            ## Create df plot ##
            df.plot.temp <- data.frame(apply(m.data, 2, median), stringsAsFactors = FALSE)
            ...
            names(df.plot.temp) <- gsub(as.vector(names(group.list)[k]),paste0(names(group.list)[k], "_", cat.name),names(df.plot.temp))
        
            
            if (k == 1) {
                df.plot <- df.plot.temp
                mDataList <- list()
                mDataList[[names(group.list)[k]]] <- m.data[,grep(paste0(names(group.list)[k], "_"), colnames(m.data))]
            } else {
                df.plot <-
                    merge(df.plot, df.plot.temp, by.x = "sample.group", by.y = "sample.group")
                mDataList[[names(group.list)[k]]] <- m.data[,grep(paste0(names(group.list)[k], "_"), colnames(m.data))]
            }
        }
        
        ## Done calculating median for all samples                           ##
        #######################################################################
        
        ## make pdf file in CWD ##
       
        df.x.axis <- unique(dfDesign[,c("sample.group.only", obj@parameterList$designTScol)])
        df.plot <-
            merge(df.plot, df.x.axis, by.x = "sample.group", by.y = "sample.group.only")

        
        ## Plot based on df.plot ##
        ## Create plotting parameters
        filename <- paste0(df.gmt[i,1], ".pdf")
        plot.headline <- paste0(gsub("_", " ", df.gmt[i,1]), " (", n.genes.in.cat,")")
        plot.headline <- paste0(plot.headline, subheadline)
        x.min <- min(df.plot[,obj@parameterList$designTScol])
        x.max <- max(df.plot[,obj@parameterList$designTScol])
        y.min <- 0.9 * min(data.matrix(df.plot[,grep(cat.name, names(df.plot))]))
        y.max <- 1.1 * max(data.matrix(df.plot[,grep(cat.name, names(df.plot))]))

        ## order df.plot ##
        ## Add x.axis to df.plot ##
        #df.x.axis <- unique(dfDesign[,c("sample.group.only", Obio@parameterList$designTScol)])
        #df.plot <-
        #    merge(df.plot, df.x.axis, by.x = "sample.group", by.y = "sample.group.only")

        
        #df.plot <- df.plot[order(df.plot$timepoint, decreasing = F),]

        ## Create summary ##
        df.add <- df.plot
        df.add$timepoint <- NULL
        
        if (i ==1){
            df.res <- df.add
        } else {
            df.add <- df.add[,c("sample.group", names(df.add)[grep(cat.name,names(df.add))])]
            df.res <- merge(df.res, df.add, by.x = "sample.group", by.y = "sample.group")
        }
        ...
        plotList[[tag]] <- ggplot(aes(x=timepoint), data=df.plot) + geom_hline(yintercept = 1, color = "black", size=0.5
                    ) + geom_vline(xintercept = sort(unique(df.plot[, Obio@parameterList$designTScol])), color = "lightgrey", size=0.5, linetype='dashed') + theme_bw()
        
        for (m in 1:length(group.list)) {
            ################################################
            ## Set color parameters 
            red <- col2rgb(ds.color[m])[1]/255
                green <- col2rgb(ds.color[m])[2]/255
                blue <- col2rgb(ds.color[m])[3]/255
                
                mPlot <- mDataList[[names(group.list)[m]]]
                
                col.names <- colnames(mPlot)[
                    grep(
                        "_median",
                        colnames(mPlot)
                    )
            ...
            ## Make ggplot ##
            ## create x-y scatterplot ##
            df.temp.plot <- df.plot
            selVec <- c(
                "sample.group",
                Obio@parameterList$designTScol,
                names(df.plot)[grep(paste0(names(group.list)[m],"_"), names(df.plot))]
            )
            
            df.temp.plot <- df.plot[,selVec]
            
            df.temp.plot[["y"]] <- df.temp.plot[,grep(paste0(names(group.list)[m],"_"), names(df.temp.plot))]
            lCol = rgb(red,green,blue,1, maxColorValue = 1)
            
            if (obj@parameterList$designTScol != "timepoint"){
              df.temp.plot[["timepoint"]] <- df.temp.plot[,obj@parameterList$designTScol]
            }
            
            plotList[[tag]] <- plotList[[tag]] + geom_line(
                data=df.temp.plot, 
                aes(x=timepoint, y=y), 
                col=lCol,se=FALSE, 
                span=spanPar
            ) + geom_point(data=df.temp.plot, aes(x=timepoint, y=y), fill=lCol, shape=21
            )
            ...
            figCap <- paste0(
                "**Figure ",
                figureCount,
                ":** Median gene expression for ", tag,". Download a pdf of this figure [here](", FNrel, "). ", link
            )
        ...
        ## End creating one plot per gmt
        ###################################
    } # end loopint through gmts
    
    
        
    returnList <- list(df.res = df.res, plotList=plotList, chnkVec = chnkVec)    
    #return(df.res)
    return(returnList)
})
...
## Correlation Cluster Table {.tabset .tabset-fade .tabset-pills}
...
## Category Enrichment Analysis on Gene Sets {.tabset .tabset-fade .tabset-pills}
